...
void *grayscale_thread(void *arg) {
    ...
    for (size_t y = data->start_row; y < data->end_row; y++) {
        ...
        for (size_t x = 0; x < img->width; x++) {
            ...
            uint8_t *r = &row_ptr[pixel_index + 0];
            uint8_t *g = &row_ptr[pixel_index + 1];
            uint8_t *b = &row_ptr[pixel_index + 2];
...
/* ---2. Grayscale Filter with Multithreading --- */
void apply_grayscale(image_t *img) {
    ...
    for(int i = 0; i < NUM_THREADS; i++) {
        ...
        if(pthread_create(&threads[i], NULL, grayscale_thread, (void *)&thread_args[i]) == -1) {
            perror("[Grayscale] Failed to create thread");  
            exit(EXIT_FAILURE);
        }
    }
